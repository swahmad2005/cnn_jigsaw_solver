...
# Represents a puzzle fragment and its connections on each side
class Fragment:
...
def unscramble(images, model_name):
    try:
        cnn_model.unscrambling_image = True # required for CNN to read single images (rather than batches)

        entries = os.listdir("./" + images + "_scrambled/") # retrieve images
        ...
        for index, file in enumerate(entries):
            ...
            with Image.open("./" + images + "_scrambled/" + file) as img:
                ...
                # Create CNN inputs (480 possible border connections)
                model = cnn_model.CNN()
                ...
                for _ in range(24):
                    min_loss = min(outputs) # next most resembling border
                ...
                for i in range(4):
                    for j in range(4):
                        if reconstructed_image[i * 4 + j] == None:
                            continue
                        piece = quadrants[reconstructed_image[i * 4 + j]] # retrieve placed image piece
